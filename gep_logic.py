...
gep.population = [Individual(gep.random_genome()) for _ in range(gep.pop_size)]
[[individual.genome[:20], individual.fitness] for individual in gep.population]
selected = [gep.binary_tournament() for _ in range(gep.pop_size)]
[[individual.genome[:20], individual.fitness] for individual in selected]
children = gep.reproduce(selected)
[[individual.genome[:20], individual.fitness] for individual in children]

child = [a if np.random.random() < 0.5 else b for a, b in zip('aaaaa', 'bbbbb')]
child
